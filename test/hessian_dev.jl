...
function lag(x::AbstractVector{Dual{T1, Dual{T2, V2, N2}, N1}}, λ) where {T1, T2, V2, N2, N1}
    ...
    b = [partials(value.(x_elem)) for x_elem in x]
    c = hcat([value.(partials(x_elem)) for x_elem in x]...)
    d = [partials.(partials(x_elem)) for x_elem in x]
...
b = [partials(value(x_elem)) for x_elem in x_in]
c = [value.(partials(x_elem)) for x_elem in x_in]
d = [partials.(partials(x_elem)) for x_elem in x_in]

T1, T2 = typeof(vals[2]).parameters[1], typeof(vals[2]).parameters[2].parameters[1]
f_a = (a.*a)'*λ
b_df_a = (2*diagm(a)'*λ)'*b
c_df_a = hcat(c...)*(2*diagm(a)'*λ)
b_ddf_a_c = (2*diagm(a)'*λ)'*d + hcat(c...)*2*diagm(λ)*b
...
c_df_a_int = [(2*diagm(a))'*c_elem for c_elem in c]
b_ddf_a_c_int = [(2*diagm(a))'*d_elem for d_elem in d] + [(2*diagm(c_elem))'*b for c_elem in c]
